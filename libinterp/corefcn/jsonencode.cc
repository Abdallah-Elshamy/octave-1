...
encode_array (T& writer, const octave_value& obj, const bool& ConvertInfAndNaN,
              const dim_vector& original_dims, int level = 0, bool is_logical = 0)
{
  NDArray array = obj.array_value ();
  // is_logical is assigned value at level 0.  I think this is better than 
  // changing many places in the code and it makes the function more modular.
  if (level == 0)
